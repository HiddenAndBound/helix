...
use helix::{
    ...
    spartan::{SpartanProof, build_default_poseidon2_instance},
};
...
fn poseidon2_spartan_bench(c: &mut Criterion) {
    ...
    c.bench_function("poseidon2_prover", |b| {
        b.iter(|| {
            ...
            let output = SpartanProof::prove_poseidon2(&instance, &mut challenger)
    ...
    let (proof, commitment) = SpartanProof::prove_poseidon2(&instance, &mut prover_challenger)
        .expect("Poseidon2 proof generation should succeed");
