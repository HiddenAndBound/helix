...
fn benchmark_unsafe_unchecked(data: &[Fp], eq_data: &[Fp], iterations: usize) -> Fp {
    ...
    debug_assert!(
        ...
        "Bounds proof failed: data.len()={} < iterations*2={}",
        data.len(),
        iterations * 2
    );
    debug_assert!(
    ...
    for (size, size_name) in sizes {
        ...
        group.bench_with_input(
            ...
            |b, (poly, eq, iter_count)| {
                b.iter(|| benchmark_unsafe_unchecked(poly, eq, *iter_count))
            },
        );
...
fn bench_sumcheck_realistic_unsafe(c: &mut Criterion) {
    ...
    group.bench_function("unsafe_unchecked_sumcheck", |b| {
        b.iter(|| benchmark_unsafe_unchecked(&poly_data, &eq_data, iterations))
    });
...
criterion_group!(
    benches,
    bench_unsafe_comparison,
    bench_sumcheck_realistic_unsafe
);
