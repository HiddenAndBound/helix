//! Sum-check protocol implementation for Spartan zkSNARK.
//!
//! Implements the sum-check protocol to prove multivariate polynomial constraints of the form:
//! `f(x₁, ..., xₙ) = ∑_{w∈{0,1}ⁿ} A(w)·B(w) - C(w)` where A, B, C are constraint matrices.
//!
//! Key optimizations:
