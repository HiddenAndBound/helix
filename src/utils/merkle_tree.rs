...
use serde::Serialize;
use sha3::{ Digest, Keccak256, Keccak256Full, digest::generic_array::GenericArray };
...
impl MerkleTree {
    ...
    pub fn from_field<D: RawDataSerializable + Copy>(leaves: &[D]) -> Result<Self> {
        ...
        for (leaf, node) in leaves.iter().zip(&mut nodes) {
            ...
            *node = Keccak256::digest(&leaf.into_bytes().into_iter().collect::<Vec<u8>>()).into();
        }
        ...
        for _d in 0..depth {
            ...
            for i in 0..next_layer_size {
                ...
                let parent_hash: [u8; 32] = Keccak256::digest(&[left, right].concat()).into();
        ...
        for _d in 0..depth {
            ...
            prev_layer
                ...
                .for_each(|(children, parent)| {
                    *parent = Keccak256::digest(children.as_flattened()).into();
                });
    ...
    pub fn verify_path(
        ...
        for (_level, &sibling_hash) in path.into_iter().enumerate() {
            ...
            let parent_hash: [u8; 32] = if is_left {
                Keccak256::digest(&[current_hash, sibling_hash].concat()).into()
            } else {
                Keccak256::digest(&[sibling_hash, current_hash].concat()).into()
            };
